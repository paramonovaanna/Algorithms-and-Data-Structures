...
struct BstNode {
  ...
  BstNode *right;
  int repeated;
  BstNode(int key, int repeated) : key(key), left(nullptr), right(nullptr), repeated(repeated) {}
...
public:
  ...
  void in_order(BstNode* now)  {
    if (now->left) {
        in_order(now->left);
    }
    std::cout << now->key << ' ' << now->repeated << '\n';
    if (now->right) {
        in_order(now->right);
    }
  }
