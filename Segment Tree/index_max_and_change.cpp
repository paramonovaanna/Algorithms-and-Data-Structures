...
using Node = std::pair<number, number>;
...
public:
  ...
  std::pair<number, number> get_max(number now, number left, number right, number left_border,
                 number right_border) {
...
int main() {
  ...
  for (size_t j = 0; j < q; ++j) {
    ...
    if (t == 1) {
        number left, right;
        std::cin >> left >> right;
        result.push_back(seg_tree.get_max(1, 0, n - 1, left, right).second);
    } else if (t == 2) {
