...
class SegmentTree {
public:
  SegmentTree(number n, numbers original) {
    n = n;
    tree.resize(4 * n);
    nums = original;
    build(1, 0, n - 1);
  ...
  number get_sum(number now, number left, number right, number left_border,
                 number right_border) {
    ...
    number middle = (left + right) / 2;
    return get_sum(2 * now, left, middle, left_border,
                   std::min(right_border, middle)) +
           get_sum(2 * now + 1, middle + 1, right,
...
int main() {
  ...
  for (size_t j = 0; j < q; ++j) {
    ...
    if (t == 1) {
        number left, right;
        std::cin >> left >> right;
        result.push_back(seg_tree.get_sum(1, 0, n - 1, left, right));
    } else if (t == 2) {
